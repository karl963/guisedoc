...
public class AdvanceInvoicePDF {
	
	Document DOC;
	
	public AdvanceInvoicePDF(Document d){
		this.DOC = d;
	}
	...
	public byte[] make(Firm firm, User user){
		...
		try{
			...
			chosenInfo.add(new Phrase(language.get("advanceInvoiceRequirement"),TR_10));
             ...
             * add language
             */
			Phrase languageSum = new Phrase(language.get("sumTotalEuro")+"    :",TR_10);
            ...
            totalSumCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
            ...
            // final sum with total+VAT
            double finalSum = totalSum+VAT;

            numbersParagraph.add(new Phrase(""+COMMA_2.format(finalSum),TR_10));
            numbersParagraph.add(Chunk.NEWLINE);  
