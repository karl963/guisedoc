...
public class OrderConfirmationPDF {
	...
	public OrderConfirmationPDF(Document d){
		this.DOC = d;
	}
	...
	public byte[] make(Firm firm, User user){
		...
		try{
			...
			// Type cell
			headerTable.addCell(new PdfPCell(new Phrase(language.get("orderConfirmation")+":  "+DOC.getFullNumber(), TR_12_B)));
			...
			if(DOC.getValidDue() != 0){ // user has set the validDue for the document
				validDueDate = language.get("dueDate")+"  " + sdf.format(new Date(gregCalendar.getTimeInMillis()+(DOC.getValidDue()*24*60*60*1000L)));
			}
			...
			PdfPCell dotsCell = new PdfPCell(new Phrase("............................................",TR_10_B));
